...
void GameLayer::ccTouchesBegan(CCSet *pTouches, CCEvent *pEvent)
{
		...
		//CCPoint tempLocation=pTouches->getLocationInView();
		...
		//	if(!tb->getUserData()&&tb->boundingBox().containsPoint(tempLocation)){
		//		ChoseTower *choseTower=new ChoseTower;
		//		choseTower->init(this,tb->getPosition());
		//		//_towers->addObject(tower);


		//		//_gold-=TOWER_COST;
		//		//ui_gold_lbl->setCString(CCString::createWithFormat("GOLD :%d",_gold)->getCString());
	...
	/*if(this->canBuyTower()){
	CCPoint tempLocation=pTouch->getLocationInView();
	tempLocation=CCDirector::sharedDirector()->convertToGL(tempLocation);
	CCObject *pObject=NULL;
	CCARRAY_FOREACH(_towerBase,pObject){
	CCSprite *tb=(CCSprite*)pObject;
	...
	tower->initWithLayer(this,tb->getPosition());
	_towers->addObject(tower);
 ...
 void GameLayer::loadTowerBase()
 {
	 ...
	 CCARRAY_FOREACH(towerPositions,pObject){
		...
		// float x=towerBasePos->objectForKey("x");
		 ...
		 //CCMenuItemImage *towerBaseMenuItem=CCMenuItemImage::create("open_spot.png","open_spot.png",this,menu_selector(GameLayer::choseTowerMenu));

		 //towerBaseMenuItem->setPosition(ccp(x,y));
		 //CCMenu *menuTowerBase=CCMenu::create();
		 //menuTowerBase->setPosition(CCPointZero);

		 //menuTowerBase->addChild(towerBaseMenuItem);

		 //this->addChild(menuTowerBase);
